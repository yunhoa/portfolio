  ...
  return (
    <section id="projects" className="section-shell">
      ...
      <p className="section-description">
        회사 프로젝트, 개인 사이드 프로젝트, 캡스톤 프로젝트를 구분해서 정리했습니다. 무엇을 만들었는지보다
        왜 만들었고 어떤 부분을 직접 맡았는지에 맞췄습니다.
      </p>

      <div className="mt-8 space-y-6">
        {projects.map((project) => (
          <article key={project.title} className="panel overflow-hidden transition hover:border-cyan-300/30">
            <div className="border-b border-slate-200 p-5 sm:p-6">
              <div className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr] lg:items-start">
                <div>
                  ...
                  {(project.organization || project.period || project.contribution) && (
                    <dl className="mt-4 grid gap-2 text-sm text-slate-600 sm:grid-cols-3">
                      {project.organization && (
                        <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2">
                          <dt className="text-xs font-semibold text-slate-500">기관</dt>
                          <dd className="mt-1 text-slate-900">{project.organization}</dd>
                        </div>
                      )}
                      {project.period && (
                        <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2">
                          <dt className="text-xs font-semibold text-slate-500">기간</dt>
                          <dd className="mt-1 text-slate-900">{project.period}</dd>
                        </div>
                      )}
                      {project.contribution && (
                        <div className="rounded-md border border-slate-200 bg-slate-50 px-3 py-2">
                          <dt className="text-xs font-semibold text-slate-500">기여도</dt>
                          <dd className="mt-1 text-slate-900">{project.contribution}</dd>
                        </div>
                      )}
                    </dl>
                  )}
