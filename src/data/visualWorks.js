...
export const visualWorks = [
  {
    ...
    outcome:
      '로봇 위치가 3D 병원 공간의 실제 위치에 맞게 표시되는 상태까지 완성했습니다. Homography는 이 프로젝트에서 처음 써봤는데, 좌표계가 다른 데이터를 맞추는 문제라면 다시 풀 수 있습니다.',
    tech: [
      'React',
      'JavaScript',
      'Three.js',
  ...
  },
  {
    ...
    improvement:
      '원인을 따라가 보니 센서 데이터가 갱신될 때마다 화면 전체가 다시 계산되고 있었습니다. 갱신과 무관한 계산까지 매번 도는 게 문제라고 보고, useMemo 메모이제이션과 설비별 데이터 매핑, 데이터 조회 로직 분리로 불필요한 재계산과 재렌더링을 줄였습니다.',
    outcome:
      '공간 정보와 제조 데이터를 한 화면에서 확인하도록 구성하고, 실시간 상태 모니터링 화면의 응답성을 개선했습니다.',
